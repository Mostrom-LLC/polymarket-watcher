  ...
  async ({ step }) => {
    ...
    for (const market of markets) {
      ...
      if (minutesUntilClose > 25 && minutesUntilClose <= 35) {
        ...
        if (!wasAlerted) {
          const recentTradesRaw = await step.run(`fetch-close-alert-trades-${market.id}`, async () => {
            ...
            for (const tokenId of market.tokenIds) {
              const { trades } = await clobApi.getTradesByToken(tokenId, { limit: 10 });
              const normalizedTrades = trades.map((trade) => {
                const normalized = clobApi.normalizeTrade(trade, market.id);
                return {
                  ...normalized,
                  outcome: normalized.outcome ?? tokenOutcomeMap.get(tokenId) ?? null,
                };
              });
          ...
          const recentTrades = (recentTradesRaw as unknown[]).map(hydrateTrade);
          const recommendationRaw = await step.run(`recommend-close-alert-${market.id}`, async () => {
            return recommender.recommendVote(market, recentTrades);
          });
